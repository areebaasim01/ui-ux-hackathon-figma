...
const Footer = () => {
  return (
    <div>
      ...
      <div className="relative top-24 xl:top-1/2 transform xl:translate-y-1/2 mt-0 xl:mt-[40px] ml-5 lg:ml-[70px] flex flex-col lg:flex-row w-[1240px] lg:w-[1155px] h-[293px] lg:h-[180px] rounded-[20px] items-center justify-between py-[30px] px-[40px] xl:py-[36px] xl:px-[64px] bg-[#000000]">
        <h1 className="w-[310px] lg:w-[540px] lg:text-center text-white text-[32px] lg:text-[38px] leading-[35px] lg:leading-[45px] font-extrabold">
        ...
        <div className="relative w-[311px] lg:w-[349px] flex flex-col space-y-4">
          ...
          <div className="relative">
            ...
            <div className="absolute left-[16px] top-[15px]">
              <Image src="/envelpoe.png" alt="Envelope Icon" width={20} height={20} />
            </div>
          </div>
          {/* Subscribe Button */}
          ...
          <div className="w-64 flex-shrink-0 md:mx-0 mx-auto text-center md:text-left">
            <div className="flex flex-col space-y-[35px]">
              ...
              <a className="flex title-font font-medium items-center md:justify-start justify-center text-gray-900">
                <span className="ml-3 text-3xl font-bold text-black">SHOP.CO</span>
              </a>
              {/* Description */}
            ...
            {["COMPANY", "HELP", "FAQ", "RESOURCES"].map((section, index) => (
              <div key={index} className="lg:w-1/4 md:w-1/2 w-full px-4">
                ...
                <nav className="list-none mb-10">
                  {section === "COMPANY" && ["About", "Features", "Works", "Career"].map((link, i) => (
                    <li key={i}><a className="text-gray-600 hover:text-gray-800">{link}</a></li>
                  ))}
                  {section === "HELP" && ["Customer Support", "Delivery Details", "Terms & Conditions", "Privacy Policy"].map((link, i) => (
                    <li key={i}><a className="text-gray-600 hover:text-gray-800">{link}</a></li>
                  ))}
                  {section === "FAQ" && ["Account", "Manage Deliveries", "Orders", "Payments"].map((link, i) => (
                    <li key={i}><a className="text-gray-600 hover:text-gray-800">{link}</a></li>
                  ))}
                  {section === "RESOURCES" && ["Free eBooks", "Development Tutorial", "How to - Blog", "YouTube Playlist"].map((link, i) => (
                    <li key={i}><a className="text-gray-600 hover:text-gray-800">{link}</a></li>
                  ))}
                </nav>
              </div>
        ...
        <div className="bg-gray-100">
          <div className="container mx-auto py-4 px-5 flex flex-wrap flex-col sm:flex-row">
            ...
            <div className="flex sm:ml-auto mt-4 sm:mt-0">
              {["Badge1.png", "Badge2.png", "Badge3.png", "Badge4.png", "Badge5.png"].map((badge, i) => (
                <Image key={i} src={`/${badge}`} alt={`Badge ${i + 1}`} height={30} width={50} />
              ))}
            </div>
          </div>
        </div>
